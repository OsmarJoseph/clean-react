import './styles.scss'

import React from 'react'

export const iconsEnum = {
  thumbDown: {
    base64:
      'data:image/png;base64,',
    color: 'red',
  },
  thumbUp: {
    base64:
      'data:image/png;base64,',
    color: 'green',
  },
}

type Props = {
  iconName: keyof typeof iconsEnum
  className?: string
}

export const Icon = ({ iconName, className }: Props): JSX.Element => {
  const icon = iconsEnum[iconName]

  return (
    <div className={`c-icon ${className}`} style-color={icon.color}>
      <img src={icon.base64} data-testid="icon" />
    </div>
  )
}
